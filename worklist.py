...
COLUMNS = (["certification_no"] + CONTEXT_COLS + ["still_missing"]
           + VALUE_COLS + ["note"])
...
def main():
    ...
    out.sort(key=lambda r: (not r["still_missing"], r["insurer"], r["plan_level"]))
    ...
    if still_needed:
        print(f"open it in Excel and fill the blank cells on the top {still_needed} row(s)")
